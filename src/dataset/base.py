...
class NBRDatasetBase(ABC):
    def __init__(
        ...
        if not os.path.exists(self.raw_path) or len(os.listdir(self.raw_path)) == 0:
            raise FileNotFoundError(f"Dataset not found in folder: {self.raw_path}. Please download it.")
    ...
    def make_leave_one_basket_split(self, test_users_rate=0.5, random_basket=False, random_state=42):
    ...
    def make_leave_two_baskets_split(self, random_baskets=False, random_state=42):
        ...
        data = self._load_and_filter_interactions()
        self._print("Splitting...")
        data = self._leave_two_baskets_split(data, random_baskets=random_baskets, random_state=random_state)
        self._print("Splitting is done")
        self._save_split(data)
    ...
    @staticmethod
    def _leave_one_basket_split(data: pd.DataFrame, test_users_rate=0.5, random_basket=False, random_state=None):
        ...
        data.loc[
            (data["user_id"].isin(test_users)) & (data["split_flag"] == "validate"),
            "split_flag",
        ] = "test"
    ...
    def _leave_two_baskets_split(data: pd.DataFrame, random_baskets=False, random_state=None):
        ...
        penult_baskets = data.loc[~data["basket_id"].isin(last_baskets)].groupby("user_id").last()["basket_id"]
    ...
    def _save_split(self, data: pd.DataFrame):
        ...
        data = data.groupby(["user_id", "split_flag", "timestamp", "basket_id"]).item_id.apply(pd.Series.tolist)
    ...
    def _filter_data(self, data: pd.DataFrame):
        ...
        while True:
            ...
            if self.min_baskets_per_user > 0:
                data = self._filter_by_count(data, "user_id", "basket_id", self.min_baskets_per_user)
        ...
        if len(data.index) < 1:
            raise RuntimeError(
                "This dataset contains no interaction after filtering. " "Please change filter setup of this split!"
            )
    ...
    def _filter_by_count(data, group_column, filter_column, min_count):
        object_count = data.groupby([group_column])[filter_column].nunique().rename("count").reset_index()
